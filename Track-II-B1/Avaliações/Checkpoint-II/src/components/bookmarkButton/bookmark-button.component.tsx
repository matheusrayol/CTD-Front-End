...
/**
 * Botão para indicação se um elemento é favorito ou não.
 * @param isBookmarked Indica se o elemento é favorito ou não.
 * @param bookmarkHandler Lida com o clique no botão para marcar/desmarcar como favorito.
 * @returns {JSX.Element}
 */
const BookmarkButton = ({ isBookmarked, bookmarkHandler }: BookmarkButtonProps): JSX.Element => {
  
  const fill = isBookmarked ? '#ff2600' : '#444444';
  const pathData = isBookmarked
  ...
  return (
    <div className="character-bookmark" onClick={bookmarkHandler}>
      <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 0 512 512" style={{ fill: `${fill}` }}>
        <path d={pathData} />
      </svg>
    </div>
  );
